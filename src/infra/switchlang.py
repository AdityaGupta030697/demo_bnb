...
class switch:
    """
        switch is a module-level implementation of the switch statement for
        Python. See https://github.com/mikeckennedy/python-switch for full
    ...
    def case(self, key, func: Callable[[], Any],
             fallthrough: Optional[bool] = False):
        """
        ...
        ```
            with switch(val) as s:
               ...
               s.default(function)
        ```
        :param key: Key for the case test (if this is a list or range, the items
                    will each be added as a case)
        :param func: Any callable taking no parameters to be executed if this
                     case matches.
        ...
        if isinstance(key, list):
            if not key:
                raise ValueError("You cannot pass an empty collection "
                                 "as the case. It will never match.")
